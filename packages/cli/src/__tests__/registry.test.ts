...
describe("Registry", () => {
  ...
  describe("prefixTailwindClasses", () => {
    ...
    it("prefixes setup integration fallback classes and keeps explicit border styles", async () => {
      const registry = await getRegistry("tw-");
      const setupIntegration = registry["setup-integration"];
      const content = setupIntegration.files
        .map((file) => file.content)
        .join("\n");

      expect(content).toContain("const defaultMessagesAreaClassName = cn(");
      expect(content).toContain(
        "tw-flex tw-min-h-0 tw-min-w-0 tw-flex-1 tw-flex-col tw-overflow-hidden"
      );
      expect(content).not.toContain(
        'messagesAreaClassName = "flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden"'
      );
      expect(content).toContain(
        "tw-border-b tw-border-solid tw-border-[var(--semantic-border-layout"
      );
      expect(content).toContain(
    ...
    it("handles data attribute selectors", async () => {
      ...
      expect(content).toContain(
        "data-[state=open]:tw-bg-[var(--semantic-bg-ui"
      );
    ...
    it("does not produce invalid tw-! prefix pattern in registry output", async () => {
      ...
      // If ! important modifiers exist, they should come BEFORE the prefix: !tw-X
      // The invalid form tw-!X should never appear
      expect(content).not.toContain("tw-!p-");
      expect(content).not.toContain("tw-!gap-");
      expect(content).not.toContain("tw-!m-");
    });

...
const meta: Meta<typeof FrustrationHandoverCard> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: `Accordion card for human escalation when callers are frustrated. The **Escalate to Human** title has an info icon with tooltip (same pattern as Knowledge Base), via \`infoTooltip\` — pass \`""\` for a non-interactive icon only. Toggle **Escalate when caller is unhappy** to show the **Prompt** textarea (shared Textarea with character count; input is capped at \`promptMaxLength\`, default 5000), then choose **Transfer to Department**. Use \`promptValidation={false}\` to disable Prompt validation and \`promptValidationMessage\` to customize the required message. Department validation is enabled by default: pass \`escalationDepartmentValidationMessage\` and the error renders immediately for an empty department. Disable it with \`escalationDepartmentValidation={false}\`. \`onEscalationPromptBlur\` fires when the Prompt field loses focus (current value passed).
    ...
    return (
      <div className="max-w-[500px]">
        <FrustrationHandoverCard
          data={data}
          onChange={(patch) => setData((prev) => ({ ...prev, ...patch }))}
          promptValidationMessage="Escalation prompt is required"
        />
      </div>
    );
  },
